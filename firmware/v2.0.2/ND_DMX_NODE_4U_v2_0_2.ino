/*
  ND DMX NODE 4U v2.0.2 TEMP + UPTIME FIX
  ...
  v2.0.2:
    - RP2040 internal temperature sensor read directly from ADC4 with trimmed
      averaging, plausibility checks and JSON-safe error reporting.
    - System uptime comes from the RP2040 64-bit hardware timer and continues
      correctly when the dashboard is closed, reopened or refreshed.
*/
...
#include "hardware/adc.h"  // RP2040 internal temperature sensor
#include "pico/time.h"     // 64-bit monotonic system uptime
...
void beginMcuTemperatureSensor() {
  // The RP2040 internal temperature diode is connected to ADC input 4.
  // The firmware owns the ADC because this project does not use any external
  // analog inputs. Initialising it once avoids relying on browser state or on
  // an implicit Arduino ADC configuration.
  adc_init();
  adc_set_temp_sensor_enabled(true);
  adc_select_input(4);

  // Let the internal sensor and ADC multiplexer settle, then discard the first
  // few conversions. The first conversion after changing ADC input can be stale.
...
float readMcuTemperatureC() {
  // Datasheet nominal transfer function:
  //   T = 27 - (Vsensor - 0.706) / 0.001721
  // The internal sensor is intended for health monitoring, not precision
  // ambient-temperature measurement.
  static float filteredTemperatureC = NAN;

  adc_set_temp_sensor_enabled(true);
  adc_select_input(4);

  // Discard conversions immediately after selecting ADC4.
  for (uint8_t i = 0; i < 4; ++i) {
    (void)adc_read();
    delayMicroseconds(20);
  }

  // A trimmed mean rejects one low and one high outlier without adding a large
  // buffer or affecting DMX timing. This routine runs only once per second.
  ...
  const float averageRaw =
      static_cast<float>(sum - minimum - maximum) /
      static_cast<float>(SAMPLE_COUNT - 2);
  const uint16_t roundedRaw = static_cast<uint16_t>(averageRaw + 0.5f);
  const float voltage = averageRaw * (3.3f / 4096.0f);
  const float measuredTemperatureC =
      27.0f - ((voltage - 0.706f) / 0.001721f);

  // At normal operating temperatures the sensor is typically around 0.54 to
  // 0.82 V. The wider raw guard catches an unselected/floating ADC channel and
  // the previous full-scale condition that produced approximately -1400 C.
  const bool rawIsPlausible = roundedRaw >= 500 && roundedRaw <= 1200;
  const bool temperatureIsPlausible =
      isfinite(measuredTemperatureC) &&
      measuredTemperatureC >= -40.0f && measuredTemperatureC <= 125.0f;

  sysLog.temperatureAdcRaw = roundedRaw;
  sysLog.temperatureSensorValid = rawIsPlausible && temperatureIsPlausible;

  if (sysLog.temperatureSensorValid) {
    // Light low-pass filtering keeps the dashboard stable while still showing
    // genuine heating within a few seconds.
    if (!isfinite(filteredTemperatureC)) {
      filteredTemperatureC = measuredTemperatureC;
    } else {
      filteredTemperatureC =
          (filteredTemperatureC * 0.75f) + (measuredTemperatureC * 0.25f);
    }
  }
...
uint64_t getSystemUptimeSeconds() {
  // time_us_64() is maintained by the RP2040 hardware timer and starts when the
  // MCU boots. It is independent of the web browser and does not suffer from
  // the approximately 49.7-day millis() rollover.
...
<div class="shell">
  ...
  <section class="panel">
    ...
    <div class="metrics">
      ...
      <div class="metric"><div class="metric-label">System Uptime</div><div class="metric-value" id="m-uptime">--</div><div class="metric-note">RP2040 time since power-on or reset</div></div>
...
let uptimeBaseSeconds=0,uptimeSyncPerformanceMs=0,uptimeAvailable=false;
...
function syncSystemUptime(seconds){const value=Number(seconds);if(!Number.isFinite(value)||value<0)return;uptimeBaseSeconds=Math.floor(value);uptimeSyncPerformanceMs=performance.now();uptimeAvailable=true;renderSystemUptime()}
function renderSystemUptime(){if(!uptimeAvailable){text('m-uptime','--');return}const elapsed=Math.floor((performance.now()-uptimeSyncPerformanceMs)/1000);text('m-uptime',fmtUptime(uptimeBaseSeconds+Math.max(0,elapsed)))}
...
function renderStatus(j){
  ...
  text('m-ip',j.ip||'--');syncSystemUptime(j.uptime);text('m-packets',packets.toLocaleString());text('m-pps',pps.toFixed(1));text('m-link',j.link?'CONNECTED':'DISCONNECTED');
  const tempText=(j.tempValid===true&&typeof j.temp==='number'&&Number.isFinite(j.temp))?j.temp.toFixed(1)+' °C':'SENSOR ERROR';
...
refreshStatus();setInterval(refreshStatus,1000);setInterval(renderSystemUptime,250);
</script>
</body>
</html>
)NDWEB";
...
void sendStatusJson(EthernetClient& client) {
  ...
  client.print(F(",\"temp\":"));
  if (sysLog.temperatureSensorValid && isfinite(sysLog.mcuTemperatureC)) {
    client.print(sysLog.mcuTemperatureC, 1);
  } else {
    // JSON has no NaN value. Use null when the RP2040 sensor is unavailable.
    client.print(F("null"));
  }
  client.print(F(",\"tempValid\":"));
  client.print(sysLog.temperatureSensorValid ? F("true") : F("false"));
  client.print(F(",\"tempRaw\":")); client.print(sysLog.temperatureAdcRaw);
...
void setup() {
  ...
  delay(1000);
  Serial.println();
  Serial.println(F("[BOOT] ND DMX NODE 4U v2.0.2"));
  Serial.println(F("[BOOT] USB CDC started"));

  pinMode(HARDWARE_LED_PIN, OUTPUT);
  digitalWrite(HARDWARE_LED_PIN, LOW);

  beginMcuTemperatureSensor();
  sysLog.mcuTemperatureC = readMcuTemperatureC();
  Serial.print(F("[BOOT] Temperature sensor: "));
  if (sysLog.temperatureSensorValid) {
    Serial.print(sysLog.mcuTemperatureC, 1);
    Serial.println(F(" C"));
  } else {
    Serial.print(F("unavailable (ADC raw "));
    Serial.print(sysLog.temperatureAdcRaw);
    Serial.println(')');
  }
