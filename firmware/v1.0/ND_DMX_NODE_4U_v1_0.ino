...
#include "hardware/sync.h" // Thư viện khóa Spinlock phần cứng của RP2040
#include "hardware/adc.h"  // Đọc cảm biến nhiệt độ nội bộ RP2040

#define HARDWARE_LED_PIN 23 

// Quản lý offset nạp chương trình PIO tránh nạp trùng gây treo Core
static int pio0_offset = -1;
static int pio1_offset = -1;

// Cờ báo hiệu hệ thống đã khởi động hoàn tất (Chống lỗi tranh chấp chéo Core)
volatile bool systemReady = false;
volatile uint32_t core1_heartbeat = 0; // Bộ đếm nhịp tim để nạp watchdog an toàn giữa 2 nhân
...
public:
    ...
    // Đẩy 1 byte vào FIFO (Không chặn luồng)
...
struct PortStatus {
  ...
  // Phát hiện xung đột IP (Conflict Detection)
  bool conflict;
  ...
  uint32_t nextFrameTimeUs;     // Thời điểm micro giây được phép gửi frame tiếp theo
  
  // Trạng thái truyền không chặn luồng
  DmxTxState state;
...
void saveConfig() {
  EEPROM.write(0, 0x51); // Magic header mới cho v8.1/8.2
...
void updateUniverseMap() {
  ...
  setupSacnSockets(); // Cập nhật lại các Socket sACN Multicast
}

// Khởi tạo các Socket sACN Multicast theo Universe của từng Port
void setupSacnSockets() {
  sacnDdp.stop();
  sacnDdp.begin(SACN_PORT); // Đón bắt Unicast sACN
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (cfg.portEnabled[i] && u > 0) {
      ...
      if (!alreadyJoined) {
        // Địa chỉ Multicast sACN chuẩn ANSI E1.31: 239.255.A.B (với Universe = A*256 + B)
...
// Tạo gói tin ArtPollReply (240 byte) gửi về cho phần mềm dò tìm tự động
void sendArtPollReply(IPAddress remoteIP)
{
    ...
    // Gửi phản hồi Unicast cho máy tính dò tìm
...
void processDmxData(int i, uint8_t* d, uint16_t len, uint8_t prio, IPAddress ip, const char* name) {
  ...
  // 1. Kiểm tra cấu hình Protocol per Port (0: AUTO, 1: ARTNET, 2: SACN)
  if (cfg.portProtocol[i] == 1 && strcmp(name, "Art-Net") != 0) return;
  if (cfg.portProtocol[i] == 2 && strcmp(name, "sACN") != 0) return;

  // 2. Phát hiện xung đột địa chỉ IP nguồn (Conflict Detection)
  ...
  // 3. Xử lý Merge Policy Mode = 2 (BLOCK)
  if (cfg.mergeMode == 2) {
    if (ports[i].active && (now - ports[i].lastPacketMs < 1000) && ip != ports[i].sender) {
  ...
  // 4. Kiểm tra mức ưu tiên sACN Priority
  ...
  // 5. Xử lý Merge Policy Mode = 1 (HTP - Highest Takes Precedence)
...
// Hàm Bóc Tách sACN Gói Tin Chuẩn ANSI E1.31
void parseSacnPacket(EthernetUDP& udp) {
  ...
  if (sacnSize >= 126 && sacnSize <= (int)sizeof(packetBuffer)) {
    ...
    // Kiểm tra ACN Packet Identifier ("ASC-E1.17" tại byte 4-12)
...
void handleNetworkPackets() {
  ...
  // 2. XỬ LÝ GÓI sACN (CẢ UNICAST VÀ MULTICAST TỪ LIGHTKEY)
...
function renderStatus(j){
  ...
  text('m-ip',j.ip||'--');text('m-uptime',fmtUptime(j.uptime));text('m-packets',packets.toLocaleString());text('m-pps',pps.toFixed(1));text('m-link',j.link?'CONNECTED':'DISCONNECTED');text('m-temp',n(j.temp).toFixed(1)+' °C');
...
void sendStatusJson(EthernetClient& client) {
  ...
  client.print(F(",\"temp\":")); client.print(sysLog.mcuTemperatureC, 1);
...
void setup() {
  ...
  delay(1000);
  Serial.println();
  Serial.println(F("[BOOT] ND DMX NODE 4U 1.0.0"));
...
void loop() {
  ...
  // Đồng bộ an toàn bảo vệ cả Nhân 1 từ Watchdog liên nhân
  static uint32_t lastWDCheck = 0;
  static uint32_t last_hb = 0;
  if (millis() - lastWDCheck >= 1000) {
    uint32_t current_hb = core1_heartbeat;
    if (current_hb != last_hb) {
      last_hb = current_hb; // Core 1 hoạt động bình thường
      watchdog_update();   // Duy trì cấp xung Watchdog an toàn
    }
    lastWDCheck = millis();
  }
  ...
  for (int i = 0; i < 4; i++) {
    ...
    // Tự động xóa cờ Conflict sau 2 giây nếu dừng trùng lặp IP
...
// ================= CORE 1: ĐỘC LẬP XUẤT TÍN HIỆU DMX =================
void setup1() {
  // Trống hoàn toàn vì phần cứng đã được Core 0 khởi tạo an toàn ở setup()
}

// Bộ máy trạng thái truyền DMX không chặn luồng, an toàn đa luồng
void handleDmxTransmit(int i) {
  ...
  switch (ports[i].state) {
    case STATE_IDLE:
      {
        ...
        // SỬA LỖI TỤT XUNG: Phát liên tục DMX 40Hz (mỗi 25ms) không bao giờ ngắt luồng ngay cả khi cảnh đứng yên
        if (time_remaining <= 0) {
          // Chế độ test chỉ thay front-buffer; dữ liệu mạng vẫn tiếp tục cập nhật trong back-buffer.
          ...
          spin_unlock(dmx_lock, irq_status); // Mở khóa ngay sau khi xử lý bộ đệm
          
          // Count the 25 ms frame period from BREAK, not from FIFO-fill completion.
          ports[i].nextFrameTimeUs = nowUs + 25000;

          // 1. Chuyển chân pin về SIO và kéo LOW để tạo xung BREAK vật lý
          pinMode(pin, OUTPUT);
    ...
    case STATE_BREAK:
      // Thời gian Break tối thiểu 88us (Đợi 105us để tín hiệu cực kỳ ổn định)
      if (nowUs - ports[i].stateStartTimeUs >= 105) {
        // 2. Kéo chân pin lên HIGH để tạo xung Mark After Break (MAB)
    ...
    case STATE_MAB:
      // Thời gian MAB tối thiểu 8us (Đợi 14us an toàn)
    ...
    case STATE_SENDING:
      // Đẩy dữ liệu vào TX FIFO không chặn luồng một cách công bằng (Giới hạn tối đa 16 byte một lượt quét)
      {
        int fed = 0;
      ...
      // Hoàn tất việc nạp dữ liệu của toàn bộ khung DMX vào FIFO
...
void loop1() {
  // Đợi hệ thống Core 0 sẵn sàng sử dụng Memory Barrier chuẩn Pico SDK
  if (!systemReady) {
    while (!systemReady) {
      asm volatile("wfe");
    }
  ...
  lCount1++; 
  core1_heartbeat++; // Tăng nhịp tim báo hiệu sống nhịp nhàng cho Core 0
